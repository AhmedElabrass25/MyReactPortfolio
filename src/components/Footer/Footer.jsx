...
const FooterBrand = () => (
  <div className="space-y-6">
    <HashLink to={"#"} className="flex items-center gap-3">
      ...
      <span className="text-xl font-black tracking-tight uppercase">Ahmed <span className="text-blue-500">Hub</span></span>
    </HashLink>
    <p className="text-gray-400 leading-relaxed max-w-xs">Building modern, scalable, and high-performance digital products that solve real-world problems.</p>
    ...
    <div className="container mx-auto px-4">
      <div className="flex flex-col lg:flex-row items-center justify-between border-b border-white/10 pb-20 mb-20 gap-10">
        <div className="max-w-2xl text-center lg:text-left">
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className="text-4xl sm:text-6xl font-black mb-6 leading-tight">
        ...
        <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} href="mailto:ahmedelabrass2025@gmail.com"
        ...
        <div className="space-y-6">
          ...
          <ul className="space-y-4">
            {["Skills", "Experience", "Features", "Main"].map((item) => (
              <li key={item}><HashLink to={`/#${item.toLowerCase()}`} className="text-gray-400 hover:text-white transition-colors flex items-center gap-2 group">
                <span className="h-px w-0 bg-blue-500 group-hover:w-4 transition-all" />{item === "Main" ? "Projects" : item === "Features" ? "Services" : item}
              </HashLink></li>
            ))}
          </ul>
        </div>
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {socialLinks.map((social, i) => (
              <a key={i} href={social.link} target="_blank" className="p-3 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-gray-400 hover:text-white hover:bg-white/10 transition-all" title={social.label}>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between pt-12 border-t border-white/10 gap-6">
        <p className="text-gray-500 text-sm font-medium">© {new Date().getFullYear()} Ahmed Mohamed. All rights reserved.</p>

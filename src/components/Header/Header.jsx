...
const navLinks = [
  { name: "About", hash: "#" }, { name: "Skills", hash: "#skills" },
  { name: "Experience", hash: "#experience" }, { name: "Services", hash: "#features" },
  { name: "Projects", hash: "#main" }, { name: "Contact", hash: "#contact" },
];
...
const Header = () => {
  ...
  useEffect(() => {
    ...
    if (mode === "dark") { body.classList.add("dark", "bg-gray-900"); body.classList.remove("light", "bg-gray-50"); }
    else { body.classList.add("light", "bg-gray-50"); body.classList.remove("dark", "bg-gray-900"); }
  }, [mode]);
  ...
  const getLinkClasses = (hash) => `relative px-4 py-2 text-sm font-bold transition-all duration-300 rounded-full flex items-center justify-center ${isActive(hash) ? "text-white bg-blue-600 shadow-lg shadow-blue-500/20 scale-105" : "text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-gray-100 dark:hover:bg-gray-800/50"}`;
  ...
  return (
    <>
      <AnimatePresence>
        {isVisible && (
          <motion.button initial={{ opacity: 0, y: 20, scale: 0.8 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.8 }}
            whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
      ...
      <nav className="sticky top-0 z-[100] w-full transition-all duration-300 bg-white/70 dark:bg-gray-900/70 backdrop-blur-xl border-b border-gray-200/50 dark:border-gray-800/50 h-20 flex items-center">
        <motion.div className="absolute bottom-0 left-0 right-0 h-[3px] bg-gradient-to-r from-blue-600 to-purple-500 origin-left z-[101]" style={{ scaleX }} />
        <div className="container mx-auto px-4 flex items-center justify-between">
          <HashLink to={"#"} className="group flex items-center gap-3">
            ...
            <span className="text-xl font-black tracking-tight text-gray-900 dark:text-white uppercase">Ahmed <span className="text-blue-600 dark:text-blue-400">Hub</span></span>
          </HashLink>
          <div className="hidden lg:flex items-center gap-2 p-1.5 bg-gray-100/50 dark:bg-gray-800/50 rounded-2xl border border-gray-200/50 dark:border-gray-700/50">
            {navLinks.map((link) => (<HashLink key={link.name} to={`/${link.hash}`} className={getLinkClasses(link.hash)}>{link.name}</HashLink>))}
          </div>
          <div className="flex items-center gap-3">
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => { const n = mode === "dark" ? "light" : "dark"; localStorage.setItem("currentMode", n); setMode(n); }}
            ...
            <motion.button whileTap={{ scale: 0.9 }} onClick={() => setIsOpened(!isOpened)}
              ...
              <div className="container mx-auto px-4 py-8 flex flex-col gap-2">
                {navLinks.map((link, i) => (
                  <motion.div key={link.name} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                    <HashLink onClick={() => setIsOpened(false)} to={`/${link.hash}`}
                      className={`flex items-center justify-between p-4 rounded-2xl text-lg font-bold transition-all ${isActive(link.hash) ? "bg-blue-600 text-white shadow-lg shadow-blue-500/20" : "text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800"}`}>
                      {link.name}

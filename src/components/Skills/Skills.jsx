...
import { FaBootstrap, FaCss3, FaGitAlt, FaGithub, FaHtml5, FaReact, FaLaravel, FaPhp } from "react-icons/fa";
import { FaSquareJs } from "react-icons/fa6";
import { RiTailwindCssFill } from "react-icons/ri";
import { SiFormik, SiMui, SiNextdotjs, SiRedux, SiTypescript, SiMysql, SiSupabase, SiPostman, SiFigma, SiZod, SiReacthookform, SiShadcnui } from "react-icons/si";
...
const skillCategories = [
  ...
  { title: "Tools & Collaboration", skills: [
...
const SkillCard = ({ skill }) => (
  <motion.div variants={itemVariants} whileHover={{ y: -5, scale: 1.02 }}
    ...
    <div className={`text-5xl mb-4 transition-transform duration-300 group-hover:scale-110 ${skill.color}`}><skill.icon /></div>
    <span className="text-sm font-bold text-gray-800 dark:text-gray-200 text-center mb-2">{skill.name}</span>
    <span className="text-[10px] px-2.5 py-1 bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 rounded-full font-bold uppercase tracking-wider group-hover:bg-blue-100 dark:group-hover:bg-blue-900/30 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{skill.level}</span>
  </motion.div>
);
    ...
    <div className="container mx-auto px-4 max-w-6xl">
      <ScrollReveal>
        <div className="text-center mb-20">
          <h2 className="text-4xl sm:text-5xl font-extrabold text-gray-900 dark:text-white mb-4">Technical Masterclass</h2>
          <p className="text-gray-600 dark:text-gray-400 text-lg max-w-2xl mx-auto font-medium">A comprehensive overview of my technical stack and expertise levels across different domains.</p>
        </div>
      </ScrollReveal>
        ...
        {skillCategories.map((category, catIndex) => (
          <ScrollReveal key={catIndex} delay={catIndex * 0.1}>
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-8 flex items-center gap-3">
              <span className="w-8 h-1.5 bg-blue-600 rounded-full" />{category.title}
            </h3>
            <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }}
              className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
              {category.skills.map((skill, i) => <SkillCard key={i} skill={skill} />)}
            </motion.div>
          </ScrollReveal>
        ))}
      </div>
